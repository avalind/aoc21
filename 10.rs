...
impl VentMap {
    ...
    fn place_line(&mut self, mut line: Line) {
        if line.horizontal() {
            ...
            // we know that the line is diagonal.
            ...
            if reverse == true {
                while counter <= dx {
                    self.add_one(line.start.x+counter, line.start.y-counter);
                    counter += 1;
                }
            } else {
                while counter <= dx {
...
fn main() {
    ...
    for line in &lines_buf {
        let mx = cmp::max(line.start.x, line.end.x);
        let my = cmp::max(line.start.y, line.end.y);
        if mx > limits.x {
                limits.x = mx;
        }
        if my > limits.y {
